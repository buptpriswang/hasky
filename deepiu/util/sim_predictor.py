...
class SimPredictor(object):
  ...
  def words_importance(self, inputs):  
    return self.sess.run(self.graph.get_collection('words_importance')[self.index], feed_dict={self.predictor.rfeed: inputs})

  def words_score(self, image):
    if self.image_model is not None:
      image = self.image_model.gen_feature(image)
    return self.sess.run(self.graph.get_collection('image_words_score')[self.index], feed_dict={self.predictor.lfeed: image}) 

  def top_words(self, image, k=1): 
    if self.image_model is not None:
      image = self.image_model.gen_feature(image)
    scores = self.graph.get_collection('image_words_score')[self.index]
    vals, indexes = tf.nn.top_k(scores, k)
    return self.sess.run([vals, indexes], feed_dict={self.predictor.lfeed: image}) 
