...
from deepiu.imtxt2txt import input
import gezi
...
class InputApp(object):
  ...
  def gen_train_input(self, inputs, decode_fn):
    ...
    image_name, image_feature, text, text_str, input_text, input_text_str = inputs(
      trainset, 
      decode_fn=decode_fn,
    ...
    return (image_name, image_feature, text, text_str, input_text, input_text_str), trainset
  ...
  def gen_valid_input(self, inputs, decode_fn):
    ...
    if FLAGS.show_eval:
      ...
      if eval_fixed:
        ...
        eval_image_feature = tf.concat([fixed_image_feature, eval_image_feature], axis=0)
  ...
  def gen_input(self, train_only=False):
    ...
    inputs, decode_fn = input.get_decodes()
