...
class PointerNetwork(object):
  def __init__(self, max_len, batch_size, num_units=32, num_attention_steps=0, input_size=1):
    self.encoder = Encoder(num_units=num_units, num_attention_steps=num_attention_steps)
  ...
  def build(self, feed_prev=False):
    encoder_outputs, final_state, encode_attention = self.encoder.encode(self.encoder_inputs)
    ...
    outputs, states, inps = self.decoder.decode(decoder_inputs, final_state, encoder_outputs, encoder_inputs, feed_prev, encode_attention)
